...
class argos(unittest.TestCase):
    ...
    def sorting_by_customer_rating(self):
        driver = self.driver
        wait = self.wait
        ...
        sort_by.click()
        customer_review = driver.find_element_by_xpath(
            "//option[contains(text(),'Customer Rating')]")
    ...
    def test_argos(self):
        ...
        games_details = self.sorting_by_customer_rating()
        ...
        try:
            with connection.cursor() as cursor:
                for i in games_details:
                    sql = "CREATE TABLE IF NOT EXISTS argos.customer_review (id varchar(50), price varchar(100), title varchar(100))"
                    cursor.execute(sql)
                    # Create a new record
                    sql = "INSERT INTO argos.customer_review (id, price, title) VALUES (%s, %s, %s)"
                    id = uuid.uuid4()
                    cursor.execute(
